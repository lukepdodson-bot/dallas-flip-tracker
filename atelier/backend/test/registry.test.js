...
process.env.ATELIER_DATA_DIR    = fs.mkdtempSync('/tmp/atelier-reg-data-');
process.env.ATELIER_STORAGE_DIR = fs.mkdtempSync('/tmp/atelier-reg-store-');
...
test('an entry verifies, and its certificate number is quotable', () => {
  ...
  assert.match(entry.public_id, /^ATL-\d{4}-[0-9A-HJKMNP-TV-Z]{4}-[0-9A-HJKMNP-TV-Z]{2}$/);
...
test('an unknown certificate number is a clean miss, not a crash', () => {
  const result = registry.verify('ATL-2026-ZZZZ-ZZ');

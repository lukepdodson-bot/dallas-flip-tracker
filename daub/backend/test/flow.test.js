...
process.env.DAUB_DATA_DIR    = fs.mkdtempSync('/tmp/daub-flow-data-');
process.env.DAUB_STORAGE_DIR = fs.mkdtempSync('/tmp/daub-flow-store-');
...
test('browsing serves the display copy, never the licensed file', async () => {
  ...
  const licensedSize = fs.statSync(`${process.env.DAUB_STORAGE_DIR}/${licensedRow.licensed_file}`).size;
